...
class Thumbnailer:
    ...
    def perform(self, video:str, output = None):
        ...
        id3tags = loads(
            check_output(['ffprobe',
                '-v', 'error',
                '-show_entries',
                'format=duration,size,bit_rate:stream=codec_name,codec_long_name,profile,codec_type,bit_rate,sample_rate,width,height,display_aspect_ratio,r_frame_rate,level,channels:stream_tags=language,title,BPS',
                '-of', 'json', video
            ]))
        duration = int(float(id3tags['format']['duration']))
        streams = [[],[],[]]
        size = int(id3tags['format']['size'])
    # fetch all required id3 tags
        for stream in id3tags['streams']:
            lang = {
                'cze': 'Czech', 'deu': 'German',
                'eng': 'English',
                'fin': 'Finnish',
                'fre': 'French',
                'hun': 'Hungarian',
            ...
            }.get(stream.get('tags', {}).get('language'), 'unknown')
            if stream['codec_type'] == 'subtitle':
                name = stream['tags'].get('title')
                if name is not None:
                    name =  name.replace(lang, '').strip().title()
                streams[2].append(
                    f'{lang}{" ("+name+")" if name and name!=lang else ""}'
                )
                continue
            bitrate = stream.get('bit_rate', {})
            if not bitrate or bitrate in ['0', '1', 'N/A']:
                bitrate = stream.get('tags', {}).get(
                    'BPS', id3tags.get('format', {}).get('bit_rate') if stream['codec_type'] == 'video' else None
                )
            ...
            # add a description to the stream list
                streams[0].append(
                    f'Video: {codec}, {bitrate + " kbps, " if bitrate else ""}{resolution} ({ratio}), at {fps:0.3f} fps ({profile}@{level})'
                )
            else:
                codec = stream.get('codec_long_name', 'unknown')
            # audio stream language
                lang = 'English' if lang == 'unknown' else lang
            # samplerate
                samplerate = truediv(float(stream['sample_rate']), 1000)
            # channels, e.g. 2, 5.1, stereo, etc
                channels = stream.get('channels', 'unknown')
            # save parsed stream as a string
                streams[1].append(f'Audio: {lang}, {bitrate + " kbps, " if bitrate else ""}{samplerate} kHz, {channels} channels, {codec})')
    # Take Snapshots
    ...
    # Resize the Image to w:1200 #
        ...
        # info
            ...
            f'{", Subtitles: " + ", ".join(streams[2][:5]) + ((" and " + str(len(streams[2])-5)+" more") if len(streams[2])>5 else "") if len(streams[2]) else ""}'
        # comments
